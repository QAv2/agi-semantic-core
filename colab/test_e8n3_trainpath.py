#!/usr/bin/env python3
"""E8-N v3 train-path micro-exercise on CPU (lane law: every new train path
...
Run:  ~/venvs/semcore/bin/python3 colab/test_e8n3_trainpath.py
"""
import json, os, sys
import numpy as np
import torch
...
# ── v3 pins: the ONE registered training change, mirrored exactly ────────────
assert "if per_strand_plateau(strand_epoch_loss," in train_cell, \
    "v3 train cell must call per_strand_plateau on the per-strand log"
assert "min_epochs=MIN_EPOCHS_V2" in train_cell and \
    "rel=PLATEAU_REL" in train_cell, "per-strand plateau args drifted"
assert "if plateau_converged(" not in train_cell, \
    "v2 pooled plateau still called — the cure did not land"
assert "EPOCHS_CAP_V3" in train_cell, "v3 cap constant not wired"
assert L.EPOCHS_CAP_V3 == 12, L.EPOCHS_CAP_V3
print("v3 pins: per-strand plateau wired, pooled call gone, cap 12 — pinned")
